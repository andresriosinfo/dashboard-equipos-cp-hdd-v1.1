...
from cp_config_manager import get_config
...
def upload_data_sql():
    """
    Establece conexión con SQL Server y ejecuta una consulta fija.
    
    Retorna:
    pd.DataFrame: Datos obtenidos de la consulta
    """
    try:
        ...
        # Get the query from configuration or use default
        query = get_config('query_datos', "SELECT * FROM dbo.nv_cp_history")
        
        # Validate connection configuration
        if not all([host, port, db_name, user, password]):
            raise ValueError("Faltan parámetros de conexión a la base de datos")
            
        logger.info(f"Conectando a la base de datos {db_name} en {host}:{port}")
        ...
        if df.empty:
            ...
            logger.info(f"Datos obtenidos: {len(df)} filas, {len(df.columns)} columnas")
        ...
        # Create dictionary of DataFrames by area
        df_dict = {}
        
        # Use the configured grouping column or default to 'area'
        grouping_col = get_config('columna_agrupacion', 'area')
        
        if grouping_col not in df.columns:
            logger.error(f"La columna de agrupación '{grouping_col}' no existe en los datos")
            # Fall back to area if it exists
            if 'area' in df.columns:
                grouping_col = 'area'
                logger.info(f"Usando 'area' como columna de agrupación por defecto")
            else:
                logger.error("No se puede agrupar los datos, no hay columna de agrupación válida")
        ...
        for variable in df[grouping_col].unique():
            ...
            logger.debug(f"DataFrame 'df_{variable}' creado con {len(df_dict[f'df_{variable}'])} filas")

        # Update metadata about the data fetch
        if db_manager.is_connected:
            db_manager.set_metadata('stats', 'last_data_fetch', 
                                  time.strftime('%Y-%m-%d %H:%M:%S'),
                                  'Last time data was fetched from the source database')
        
        return df_dict
        
    except Exception as e:
        logger.error(f"Error al obtener datos de SQL Server: {str(e)}")
        import traceback
